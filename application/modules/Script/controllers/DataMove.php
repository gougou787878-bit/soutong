...
$d1 = new Medoo([
    ...
    'database_type' => 'mysql',
    'database_name' => "{$sDb}",
    'server' => '172.104.110.12',
    'username' => 'ks_master',
    'password' => 'pArm*%anEntF]1',
]);
...
$d2 = new Medoo([
    ...
    'database_type' => 'mysql',
    'database_name' => "{$tDb}",
    'server' => '139.162.66.189',
    'username' => 'root',
    'password' => 'pArm*%anEntF]',
]);

// 插入主键id


/*$users = $d1->select("cmf_users", "id",[
    "openid" => 'live'
]);
...
foreach ($users as $v) {
    echo "{$v}\n";
   $result =  $d2->insert("ks_members_copy", [
        "uid" =>intval($v)
    ]);
...
// 更新 'oauth_type'
/*$users = $d1->select("cmf_users", ['id','source'],[
    "openid" => 'live'
]);
foreach ($users as $v) {
    echo "to do \n";
    $d2->update("ks_members_copy", [
       'oauth_type'=>"{$v['source']}",
        //'oauth_t$ype' =>
    ],[ "uid" =>"{$v['id']}"]);
}*/
...
/*$users = $d1->select("cmf_users", ['id', 'user_login', 'mobile', 'sex', 'birthday', 'signature'], [
    "openid" => 'live'
]);
foreach ($users as $v) {
    echo "to do \n";
    $d2->update("ks_members_copy", [
        'oauth_id' => "{$v['user_login']}",
        'uuid' => md5($v['id']),
        'username' => $v['mobile'],
        'phone' => $v['mobile'],
        'birthday' => $v['birthday']
...
// 更新 'vip'
/*$users = $d1->select("cmf_users_vip", ['uid', 'endtime'], [
    'endtime[>=]' => time()
]);
foreach ($users as $v) {
    echo "to do \n";
    $vip_level = 1;
    if ($v['endtime'] < time() + 86400 * 30) {
        $vip_level = 1;
    } elseif ($v['endtime'] < time() + 86400 * 30 * 6) {
        $vip_level = 2;
    } elseif ($v['endtime'] < time() + 86400 * 30 * 6) {
        $vip_level = 3;
    }
    $d2->update("ks_members_copy", [
        'expired_at' => $v['endtime'],
        'vip_level' => $vip_level,
        //'oauth_t$ype' =>
    ], ["uid" => "{$v['uid']}"]);
}*/
...
// 更新 'consumption,votes,votes_total,signature'

// 可以平移的表

/*$users = $d1->select("cmf_car", ['id',
    'name',
    'thumb',
    'swf',
    'swftime',
...
/*$users = $d1->select("cmf_users_car", ['id',
    'uid',
    'carid',
    'endtime',
    'status',
...
foreach ($users as $v) {
    ...
    $d2->insert("ks_guard", [
        ...
        'type'=>$v['type'],
        'coin'=>$v['coin'],
        'length_type'=>$v['length_type'],
        'length'=>$v['length'],
        'length_time'=>$v['length_time'],
        'addtime'=>$v['addtime'],
        'uptime'=>$v['uptime'],
        'orderno'=>$v['orderno'],
    ]);
...
/*$users = $d1->select("cmf_guard_users", [
    'id',
    'uid',
    'liveuid',
    'type',
...
/*$users = $d1->select("cmf_users_auth", ['uid',
    'real_name',
    'mobile',
    'cer_no',
    'front_view',
...
foreach ($users as $v) {
    ...
    $d2->insert("ks_auth", [
        ...
        'mobile' => $v['mobile'],
        'cer_no' => $v['cer_no'],
        'front_view' => $v['front_view'],
        'back_view' => $v['back_view'],
        'handset_view' => $v['handset_view'],
        'reason' => $v['reason'],
        'addtime' => $v['addtime'],
        'uptime' => $v['uptime'],
        'status' => $v['status'],
...
/*$users = $d2->select("ks_auth", ['uid', 'status']);

var_dump($users);
foreach ($users as $v) {
    echo "to do \n";
    // if ($v['status'] == 1) {
        $d2->update("ks_members", [
            'auth_status' => 1,
            //'oauth_t$ype' =>
        ], ["uid" => "{$v['uid']}"]);
  //   }
...
// 最后变更
/*$users = $d1->select("cmf_users", ['id','consumption','votes','votestotal','coin'], [
    "openid" => 'live'
]);
foreach ($users as $v) {
    echo "to do \n";
...
foreach ($users as $v) {
    ...
    $d2->insert("ks_family", [
        ...
        'name'=>$v['name'],
        'badge'=>$v['badge'],
        'apply_pos'=>$v['apply_pos'],
        'apply_side'=>$v['apply_side'],
        'briefing'=>$v['briefing'],
        'carded'=>$v['carded'],
        'fullname'=>$v['fullname'],
        'addtime'=>$v['addtime'],
...
/*$users = $d1->select("cmf_family_profit", [
    'id',
    'uid',
    'familyid',
    'time',
...
foreach ($users as $v) {
    echo "to do \n";
    // if ($v['status'] == 1) {
    $d2->insert("ks_family_profit", [
        'id'=>$v['id'],
        'uid'=>$v['uid'],
        'familyid'=>$v['familyid'],
        'time'=>$v['time'],
        'profit_anthor'=>$v['profit_anthor'],
        'total'=>$v['total'],
        'profit'=>$v['profit'],
        'addtime'=>$v['addtime'],
...
$users = $d1->select("cmf_users_family", [
    'id',
    'uid',
    'familyid',
    'addtime',
...
foreach ($users as $v) {
    ...
    $d2->insert("ks_family_member", [
        ...
        'signout'=>$v['signout'],
        'istip'=>$v['istip'],
        'signout_reason'=>$v['signout_reason'],
        'signout_istip'=>$v['signout_istip'],
        'divide_family'=>$v['divide_family'],
    ]);
    //   }
}