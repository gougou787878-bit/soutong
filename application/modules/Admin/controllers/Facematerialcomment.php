...
/**
 * Class FacematerialcommentController
 * @author xiongba
 * @date 2023-06-09 20:10:36
 */
class FacematerialcommentController extends BackendBaseController
{
    /**
     * 列表数据过滤
     * @return Closure
    ...
    {
       return FaceMaterialCommentModel::class;
    }
    ...
    public function doPassAction()
    {
        ...
        $post = $this->postArray();
        /** @var FaceMaterialCommentModel $model */
        $model = FaceMaterialCommentModel::where('id', $post['id'])->first();
        if (is_null($model)) {
            return $this->ajaxError('数据不存在');
        }
        try {
            $model->update(['status' => FaceMaterialCommentModel::STATUS_PASS]);
            //维护帖子评论数
            $post = FaceMaterialModel::where('id',$model->material_id);
            if ($post){
                $post->increment('comment_num');
                //清理缓存
                FaceMaterialCommentModel::clearCacheWhenCreatePostComment($model->material_id);
            }
    ...
    public function doRejectAction()
    {
        ...
        $post = $this->postArray();
        $flag = FaceMaterialCommentModel::where('id', $post['id'])->update(['status' => FaceMaterialCommentModel::STATUS_UNPASS]);
    ...
    public function doBlackAction(){
        ...
        $flag = $post['flag']??'';
        /** @var FaceMaterialCommentModel $model */
        $model  = FaceMaterialCommentModel::find($post['id']);
        if(!is_null($model)){
            if($flag == 'ban'){
                //下掉VIP权益
                MemberModel::ban($model->aff);
                FaceMaterialCommentModel::where(['aff'=>$model->aff])->delete();
            }
            if($flag == 'black'){
    ...
    public function passAllAction()
    {
        ...
        $comments = FaceMaterialCommentModel::whereIn('id', $ary)
            ->where('status', FaceMaterialCommentModel::STATUS_WAIT)
            ->get();

        try {
            transaction(function () use ($comments) {
                foreach ($comments as $comment) {
                    $ret = FaceMaterialCommentModel::where('id', $comment->id)
                        ->update(['status' => FaceMaterialCommentModel::STATUS_PASS]);
                    if ($ret <= 0)
                        throw new Exception('系统异常');

                    $postId = $comment->material_id;
                    $post = FaceMaterialModel::find($postId);
                    if ($post){
                        $post->increment('comment_num');
                        //清理缓存
                        FaceMaterialCommentModel::clearCacheWhenCreatePostComment($postId);
                    }
...
//                            $post = FaceMaterialModel::where('id', $comment->material_id)->first();
//                            $autherAff = $post->aff;
...
//                            // 对评论人通知评论
//                            $nickname = MemberModel::firstAff($comment->aff)->nickname;
//                            $tcomment = FaceMaterialCommentModel::where('id', $comment->pid)->first();
...
//                    }
    ...
    public function batchRefuseAction(){
        try {
            ...
            //过滤已经审核通过的
            $comments = FaceMaterialCommentModel::query()->whereIn('id',$commentIds)->where('status',FaceMaterialCommentModel::STATUS_WAIT)->get();
            if (!$comments){
            ...
            foreach ($comments as $comment) {
                $data = [
                    'status'       => FaceMaterialCommentModel::STATUS_UNPASS,
