...
class PostcommentController extends BackendBaseController
{
    ...
    public function doPassAction()
    {
        ...
        $model = PostCommentModel::where('id', $post['id'])->first();
        if (is_null($model)) {
        ...
        try {
            ...
            //维护帖子评论数
            $post = PostModel::where('id',$model->post_id);
            if ($post){
                $post->increment('comment_num');
                //清理缓存
                PostCommentModel::clearCacheWhenCreatePostComment($model->post_id);
            }
    ...
    public function doRejectAction()
    {
        ...
        $post = $this->postArray();
        $flag = PostCommentModel::where('id', $post['id'])->update(['status' => PostCommentModel::STATUS_UNPASS]);
    ...
    public function doBlackAction(){
        ...
        $flag = $post['flag']??'';
        /** @var PostCommentModel $model */
        $model  = PostCommentModel::find($post['id']);
        if(!is_null($model)){
            if($flag == 'ban'){
                //下掉VIP权益
                MemberModel::ban($model->aff);
                PostCommentModel::where(['uid'=>$model->aff])->delete();
            }
            if($flag == 'black'){
    ...
    public function passAllAction()
    {
        ...
        $comments = PostCommentModel::whereIn('id', $ary)
            ->where('status', PostCommentModel::STATUS_WAIT)
            ->get();
            ...
            transaction(function () use ($comments) {
                foreach ($comments as $comment) {
                    $ret = PostCommentModel::where('id', $comment->id)
                        ->update(['status' => PostCommentModel::STATUS_PASS]);
                    if ($ret <= 0)
                    ...
                    if ($post){
                        $post->increment('comment_num');
                        //清理缓存
                        PostCommentModel::clearCacheWhenCreatePostComment($postId);
                    }
