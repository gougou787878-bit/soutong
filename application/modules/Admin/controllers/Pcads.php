...
class PcadsController extends BackendBaseController
{
    ...
    {
        if(!is_null($model)){
            PcAdsModel::clearRedisCache($model->position);
        }
    ...
    {
        $p = $this->postArray();
        $model = PcAdsModel::where('id',$p['_pk'])->first();
        if(!is_null($model)){
            PcAdsModel::clearRedisCache($model->position);
        }
    ...
    private function deleteAfterCallback($model, $isDelete)
    {
        if ($isDelete){
            ...
            $new_pos = PcAdsModel::POSITION[$model->position];
            $new_status = PcAdsModel::STATUS[$model->status];
            $new_link = $model->url;
            $ads_type = PcAdsModel::ADS_TYPE[$model->type];
...
MSG;
    ...
    {
        $username = $this->getUser()->username;
        $old_pos = PcAdsModel::POSITION[$oldModel->position];
        $new_pos = PcAdsModel::POSITION[$model->position];
        $old_status = PcAdsModel::STATUS[$oldModel->status];
        $new_status = PcAdsModel::STATUS[$model->status];
...
MSG;
    ...
    public function delAction() {
        ...
        $post = $this->postArray();
        $model = PcAdsModel::where('id', $post['_pk'])->first();
        if (empty($model)){
    ...
    {
        foreach (PcAdsModel::POSITION as $_p){
            PcAdsModel::clearRedisCache($_p);
        }
