...
class StoryService
{
    ...
    {
        StoryModel::setWatchUser($member);
        /** @var StoryModel $detail */
        $detail = StoryModel::getRow($id);
        test_assert($detail, '查无小说信息');
        $detail->load(['series'=>function($query){
            return $query->select(['id','story_id','series','title','created_at']);
        }]);
        $detail->now_total= 0 ;//目前总章节
        if ($detail->series) {
            $detail->now_total = collect($detail->series)->count();
        }
        jobs([StoryModel::class, 'addView'], [$id]);
        return $detail;
    }
    ...
    {
        /** @var StoryModel $detail */
        $detail = StoryModel::getRow($m_id);
        if (is_null($detail)) {
            throw new \Exception("查无小说信息");
        }
        $detail = $detail->watchByUser($member);
        $detail->load('series')->series;
        $now_total = collect($detail->series)->count();
        if ( ($member->vip_level && $member->is_vip && $detail->coins==0 )||
            ($detail->is_pay) ||
            ($now_total>5 && $s_id<=2)
        ) {
            return \StorySeriesModel::getSeriesSrc($m_id, $s_id);
        }
        throw new \Exception("无权查看小说章节详细信息~");
    }
    ...
    static function guessByManHuaLike($manhua_id, $limit = 18)
    {
        ...
        $data = StoryModel::queryBase()->when($where, function ($query, $value) {
    ...
    static function getOrderData($comics_id)
    {
        ...
        /** @var StoryModel $comics */
        $comics = StoryModel::getRow($comics_id);
        if (is_null($comics)) {
        ...
        $has_pay = \StoryPayModel::hasBuy($member->uid, $comics->id);
        if ($has_pay) {
    ...
    static function getFavorites($comics_id)
    {
        ...
        /** @var StoryModel $has */
        $has = StoryModel::getRow($comics_id);
        if (!$has) {
        ...
        $user_id = $member->uid;
        /** @var \StoryFavoritesModel $hasLike */
        $hasLike = \StoryFavoritesModel::hasLike($member->uid, $has->id);

        if ($hasLike) {
            \StoryFavoritesModel::where(['id' => $hasLike->id])->delete();
            $msg = "收藏取消";
            return [
                'status' => 0,
                'msg'    => $msg
    ...
    public static function searchManhua($postData, $is_filter = false)
    {
        ...
        if ($is_filter) {
            ...
            $query->when($type == 'doing', function ($query, $value) {
                return $query->where('is_finish', '=', StoryModel::FINISH_NO);
            })->when($type == 'finish', function ($query, $value) {
                return $query->where('is_finish', '=', StoryModel::FINISH_YES);
            })->when($type =='short', function ($query, $value) use ($type) {
                //return $query->where('type','=',trim($type));
                return $query->where([
                    ['is_finish', '=', StoryModel::FINISH_YES],
                    ['newest_series', '=', 1],
                ]);
            ...
            if ($tab_id = $postData['tab'] ?? '') {
                $tagString = \StoryTabModel::getMatchString($tab_id);
    ...
    const TAB_LIST_KEY = 'tab:list:story';
    const TAB_SEARCH_KEY = 'tab:filter:story';

    static function clearCache($id)
    {
        redis()->del("t:story:{$id}");
    ...
    public static function getTabList()
    {
        $list = cached(self::TAB_LIST_KEY)->clearCached()->fetchJson(function () {
            return \StoryTabModel::queryBase(['is_tab' => 1])
                ->orderBy('sort_num')
                ->limit(6)
    ...
    public static function getSearchList()
    {
        $list = cached(self::TAB_SEARCH_KEY)
            ->fetchJson(function () {
                return \StoryTabModel::queryBase(['is_category' => 1])
    ...
    static function getHomeData()
    {
        ...
        array_unshift($cateTabData, [
            ...
            'tags_str'    => '',
            'tags_ary'    => [],
            'show_style'  => \StoryTabModel::SHOW_STYLE_DF,
            'show_number' => \StoryTabModel::SHOW_NUMBER_DF,
            'show_type' => \StoryTabModel::SHOW_STYLE_TYPE[\StoryTabModel::SHOW_STYLE_DF],
        ]);
        $data = collect($cateTabData)->map(function ($_cateTab) {
            $data = cached("story:cat:{$_cateTab['tab_id']}")->clearCached()->fetchJson(function () use ($_cateTab) {
                return StoryModel::queryBase()->where(function ($query) use ($_cateTab) {
                    ...
                    ->limit(max(\StoryTabModel::SHOW_NUMBER_DF, $_cateTab['show_number']))
                    ->get()
                    ->toArray();
            }, rand(1000, 2000));//过期随机 避免同时穿透
            $_cateTab['items'] = $data;
            $_cateTab['show_style'] = empty($_cateTab['show_style']) ? \StoryTabModel::SHOW_STYLE_DF : $_cateTab['show_style'];
            $_cateTab['show_number'] = max(\StoryTabModel::SHOW_NUMBER_DF, $_cateTab['show_number']);
            $_cateTab['show_type'] = \StoryTabModel::SHOW_STYLE_TYPE[$_cateTab['show_style']];
            return $_cateTab;
        })->toArray();
        return $data;
    }
