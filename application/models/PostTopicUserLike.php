 ...
 * @mixin \Eloquent
 */
class PostTopicUserLikeModel extends Model
{

    protected $table = "post_topic_user_like";
    ...
    {
        return $this->hasOne(PostTopicModel::class, 'id', 'related_id');
    }
    ...
    public static function listTopicFollow(MemberModel $member, $page, $limit)
    {
        $with = [
            'topic' => function ($q) {
                $q->select(['id', 'name', 'thumb', 'bg_thumb'])->where('status', PostTopicModel::STATUS_NORMAL);
            }
        ];
