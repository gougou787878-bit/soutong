 ...
 * @mixin \Eloquent
 */
class GirlModel extends EloquentModel
{
    ...
    public function getIsFollowAttribute(): int
    {
        ...
        $flag = GirlUserLikeModel::getIdsById($this->watchUser->getAttributeValue('uid'), $this->getAttributeValue('id'));
        return $flag ? 1 : 0;
    }
    public function getPriceAttribute(): int
    {
        if($this->attributes['price'] > 10 ){
            return $this->attributes['price'];
        }else{
            return self::UNLOCK_FEE;
        }
    }
    ...
    public function getIsPayAttribute()
    {
        ...
        if ($this->price) {
            ...
            $flag = GirlPayLogModel::hasBuy($this->watchUser->uid, $this->getAttribute('id'));
            return $flag ? 1 : 0;
        } else {
    ...
    const GIRL_TOPIC_LIST_KEY = 'girl:topic:post:list:%s:%s:%s:%s';
    ...
    // GROUP
    const GIRL_TOPIC_LIST_GROUP_KEY = 'girl_topic_post_list';
    ...
    const GIRL_DETAIL_GROUP_KEY = 'girl_detail';
    const GIRL_DETAIL_KEY = 'girl:detail:%s';
    ...
    const CK_GIRL_PEER_AFF_LIST_KEY = 'ck:girl:peer:aff:list:key:%s:%s:%d:%d';
    const GP_GIRL_PEER_AFF_LIST_KEY = 'gp:girl:peer:aff:list:key';
    ...
    {
        return $this->hasOne(GirlTopicModel::class, 'id', 'topic_id');
    }

    public function medias(): \Illuminate\Database\Eloquent\Relations\HasMany
    {
        return $this->hasMany(GirlMediaModel::class, 'pid', 'id');
    }
    ...
    {
        redis()->del(sprintf(self::GIRL_DETAIL_KEY, $postId));
    }
    ...
    {
        return cached(sprintf(self::GIRL_DETAIL_KEY,$id))
            ->group(self::GIRL_DETAIL_GROUP_KEY)
            ->chinese('约炮详情')
    ...
    {
        $cacheKey = sprintf(\GirlModel::GIRL_TOPIC_LIST_KEY, $cate, $topicId, $page, $limit);
        $group = self::GIRL_TOPIC_LIST_GROUP_KEY;
        return cached($cacheKey)
            ->group($group)
            ->chinese("社区帖子列表")
            ->fetchPhp(function () use ($cate, $topicId, $page, $limit) {
                 return GirlModel::queryBase()
                     ->with('topic:id,name')
                     ->with('medias:pid,media_url,cover,thumb_width,thumb_height,type,status')
                    ...
                    ->when($cate == 'choice', function ($q) {
                        return $q->where('is_best',\GirlModel::BEST_OK);
                    })
                    ...
                    ->orderByDesc('id')
                    ->forPage($page,$limit)
                    ->get()->map(function (\GirlModel $item){
                        //列表不缓存内容字段
                         $item->content = '';
                         return $item;
                     });
    ...
    public static function listMemberPosts($aff, $kwy, $page, $limit)
    {
        ...
        $cacheKey = sprintf(self::CK_GIRL_PEER_AFF_LIST_KEY, $aff, $kwy_key, $page, $limit);
        return cached($cacheKey)
            ->group(self::GP_GIRL_PEER_AFF_LIST_KEY)
            ->chinese("他人中心列表")
