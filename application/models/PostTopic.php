 ...
 * @mixin \Eloquent
 */
class PostTopicModel extends EloquentModel
{
    ...
    const CK_TOPIC_ALL = 'ck:post:topic:all:%s:%s';
    const GP_TOPIC_ALL = 'gp:post:topic:all';
    const CN_TOPIC_ALL = '话题分页';

    const POST_TOPIC_HOT_LIST_KEY = 'post:hot:tab';
    ...
    public function getIsFollowAttribute(): int
    {
        ...
        $flag = PostTopicUserLikeModel::hasFollowedTopic($watchUser->getAttributeValue('uid'), $this->getAttributeValue('id'));
        return $flag ? 1 : 0;
    }
    ...
    public static function getTopicById($topicId, $member = null)
    {
        ...
        if ($topic) {
            $topic = PostTopicModel::makeOnce($topic);
        }
        return $topic;
    }
    ...
    {
        return cached(self::POST_TOPIC_HOT_LIST_KEY)->group(self::POST_TOPIC_ALL_GROUP_KEY)
