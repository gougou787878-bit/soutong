...
class PostMediaModel extends Model
{
    ...
    protected $fillable = ['media_url', 'cover', 'thumb_width', 'thumb_height', 'pid', 'aff', 'type', 'created_at', 'status', 'duration', 'updated_at', 'relate_type'];
    ...
    public function post(){
        return self::hasOne(PostModel::class,'id','pid');
    }
    ...
    /*public function getMediaUrlFullAttribute()
    {
        ...
        if($media_url){
            ...
            if($extension == 'mp4'){
                ...
                return getPlayUrl($media_url,true);
            }else{
                return url_cover($media_url);
            }
        }
        return '';
    }*/
    ...
    static function approvedMv($model)
    {
        ...
        $data['sign'] = $sign;
        $data['notifyUrl'] =SYSTEM_NOTIFY_SLICE_POST_URL;
        if ('test' == APP_ENVIRON){
            $data['notifyUrl'] ='https://sky.hyys.info/index.php?&m=mv&a=post_media';
        }
