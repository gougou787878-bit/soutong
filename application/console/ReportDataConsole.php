...
class ReportDataConsole extends AbstractConsole
{

    /**
     * @var string 定义同步命令
     */
    public $name = 'report-data';
    /**
     * @var string 定义命令描述
     */
    public $description = '渠道上报数据';
    ...
    public function process($argc, $argv) {
        ...
        if($noDaemon == 'nodaemon'){
            echo "start nodaemon report-data \r\n";
            $service = new AppCenterService();
            $service->startDeamonReportData();
            echo "\r\nover\r\n";
            return ;
        }
        echo "start daemonize report-data \r\n";
        $this->daemonize();
    ...
    {
        (new AppCenterService())->startDeamonReportData();
    }
        ...
        return function ($pid) {
            while (1) {
                ...
                $this->fork(function ($_pid) use (&$pid) {
                    ...
                    cli_set_process_title('report-data worker');
                    $this->workerProcess();
                });
                usleep(50000);
            }
    ...
    public function daemonize()
    {
        ...
        $this->fork($this->getForkDie(), null);
        cli_set_process_title('report-data Master');
    }
