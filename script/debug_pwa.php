...
define('appId', '68ccdfadf6ffddcgbsd2dgc9077f8c62');
define('appKey', 'cc88ddc9357ff461e08f047aedee692b');
define('encryptKey', 'e89225cfbbimgkcu');
define('SERVER_IP', '127.0.0.1');
define('SERVER_URL', 'http://gay.me');
define('SERVER_HOST', 'gay.me');
$global = [
    "bundle_id"  => "tips.yc.bs",
    'version'    => '1.0.0',
    'oauth_type' => 'pwa',
    'oauth_id'   => '114d7daffe70738210cdb08b40e97f75',
];
...
try {
    ...
    defined('APP_TYPE_FLAG') or define('APP_TYPE_FLAG',0);//pwa入口
...
$uri = str_replace(basename(__FILE__), "/pwa.php", ltrim($_SERVER['REQUEST_URI'], '/'));
...
//var_dump($result);die;
    ...
    {
        $data = openssl_encrypt($input, 'aes-256-cfb', self::appKey, OPENSSL_RAW_DATA, self::encryptKey);
    ...
    {
        $input = hex2bin($input);
        $decrypted = openssl_decrypt($input, 'aes-256-cfb', self::appKey, OPENSSL_RAW_DATA, self::encryptKey);
        return $decrypted;
    }
    ...
    {
        if (self::DEBUG) {
            $log_name = APP_PATH.'/storage/logs/log_pwa.txt';
