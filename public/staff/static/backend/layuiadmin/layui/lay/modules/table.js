...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function (exports) {
    ...
    Class.prototype.reload = function (options) {
        ...
        if (options.hasOwnProperty('orderBy')){
            if (that.config.hasOwnProperty('where')){
                that.config.where["orderBy"] = options['orderBy'];
            }
        }
    ...
    Class.prototype.renderData = function (res, curr, count, sort) {
                        ...
                        var td = ['<td data-field="' + field + '" data-key="' + key + '" ' + function () { //追加各种属性
                            ...
                            , '<div class="layui-table-cell laytable-cell-' + function () { //返回对应的CSS类标识
                                ...
                                var tplData = $.extend(true, {
                                    ...
                                    format = item3.format || false,
                                    enumJson = item3.value || {};

                                if (item3.type === 'date' || item3.type === 'datetime'|| item3.type === 'time') {
                                    var dateObject = new Date(tplData[item3.field] * 1000),
                                ...
                                switch (item3.type) {
                                    case 'checkbox':
                                        ...
                                        break;
                                    case 'enum':
                                        return enumJson[tplData[item3.field]];
                                    case 'datetime':
                                        return isoDate + ' ' + (dateObject+"").substr(16, 8);
                                    case 'time':
                                        return (dateObject+"").substr(16, 8);
                                    case 'date':
                                        return isoDate;
                                    case 'switch':
                                        return '<input name="layTableSwitch" ' + (item3.edit ? '' : 'disabled') + ' type="checkbox" ' + (function () {
                                            if (!format){
                                                format = function (d,_v) {
                                                    return d[item3.field] == (item3.value || item3.text || "ON|OFF").split('|')[0];
                                                }
                                            }
                                            var result = [
                                                format(tplData) ? 'checked' : ''
                                            ];
                                            return result.join(' ');
                                        })() + ' lay-skin="switch"  lay-text="' + (item3.text || "ON|OFF") + '" lay-value="' + (item3.value || "ON|OFF") + '">';
                                        break;
                                    case 'img':
                                        var w = item3.eleWidth || 'auto', h = item3.eleHeight || 'auto';
                                        if (!format){
                                            format = function (d,_v) {
                                                return d[item3.field];
                                            }
                                        }
                                        return "<img src='" + format(tplData) + "' width='" + w + "' height='" + h + "' style='" + (item3.css || '') + "'/>";
                                    case 'radio':
                            ...
                            , '</div>'
                            , function () {
                                var data = $.extend(true, {LAY_INDEX: numbers}, item1)
                                    , format = item3.format || function (d) {
                                        return d[item3.field]
                                    }
                                    , colConfig = item3,
                                    option = function () {
                                        console.log(colConfig.presetValue);
                                        var preset = colConfig.presetValue || [], result = [], i, item, _value;

                                        if (preset instanceof Array){
                                            for (i = 0 , item = {}; i < preset.length; i++) {
                                                item = preset[i];
                                                if (item['text'] || item['value']) {
                                                    ...
                                                    _value = item['value'];
                                                    result.push('<option value="' + (item["value"] || item["text"]) + '" ' + (_value == data[colConfig.field] ? 'selected' : '') + ' >' + (item["text"] || item["value"]) + '</option>')
                                                }
                                            }
                                        }else{
                                            for (let key in preset) {
                                                result.push('<option value="' + key + '" ' + (key == data[colConfig.field] ? 'selected' : '') + ' >' + (preset[key]) + '</option>')
                                            }
                                        }
                                        return result.join("");
                                    };

                                var filter = that.key + '-' + ELEM_EDIT + '-' + i3 + '-' + i1,
                                    filterFrom = that.key + '-' + ELEM_EDIT + '-form-' + i3 + '-' + i1,
                                    classForm = ELEM_EDIT + '-form';

                                //渲染不同风格的列
                                if (item3.type === 'select') {
                                    var html = '<form class="layui-form ' + classForm + ' ' + filterFrom + '" lay-filter="' + filterFrom + '" style="position: absolute;top: 0;left: 0;width: 100%;display: none">' +
                                        '<select lay-filter="' + filter + '" name="layTableSelect">' +
                                        option() +
                                        '</select></form>';
                                    return html;
                                }
        ...
        if (options.page) {
            ...
            if (res.data.length >= options.page.limit) {
                options.page.count = 99999999999;
            } else {
                options.page.count = (options.page.curr - 1) * options.page.limit + res.data.length;
            }
            ...
            // layui.each(data, function (i1, item1) {
            //     that.eachCols(function (i3, item3) {
            //         if (item3.edit && item3.type === 'switch'){
            //             var field = item3.field || i3
            //                 , filterSwitch = that.key + '-' + ELEM_EDIT + '-' + i3 + '-' + i1
            //
            //             form.on('switch(' + filterSwitch + ')', function (o) {
            //                 o.value = o.othis.text();
            //                 layui.event.call(this, MOD_NAME, 'edit(' + $('#' + that.key).attr('lay-filter') + ')', {
            //                     value: o.value
            //                     , field: field
            //                 });
            //             });
    ...
    Class.prototype.scrollPatch = function () {
        ...
        //修复高度
        var fixRTr = that.layFixRight.find('.layui-table>tbody>tr'),
            fixLtr = that.layFixLeft.find('.layui-table>tbody>tr');
        that.layMain.find('.layui-table>tbody>tr').each(function (k , item) {
            fixRTr.eq(k).css('height' , $(item).height() + 'px');
            fixLtr.eq(k).css('height' , $(item).height() + 'px');
        })
    };
    ...
    Class.prototype.events = function () {
        ...
        //数据切换
        that.elem.on('click', 'div.layui-form-switch', function () {
            var othis = $(this)
                , value = $(this).text()
                , field = othis.parents('td').eq(0).data('field')
                , edit = othis.parents('td').eq(0).data('edit')
                , index = othis.parents('tr').eq(0).data('index')
                , data = table.cache[that.key][index]
                , valueArray = othis.prev().attr('lay-value')
                , textArray = othis.prev().attr('lay-text');
            ...
            if (valueArray) {
                var ary = valueArray.split('|'),
                    tArr = textArray.split('|');
        ...
        //单元格编辑
        that.layBody.on('change', '.' + ELEM_EDIT + "_select,." + ELEM_EDIT, function () {
        ...
        that.layBody.on('click', 'td', function (e) {
            var othis = $(this)
                ...
                , classSelect = ELEM_EDIT + '-form'
                , classSwitch = ELEM_EDIT + '-switch';

            if (othis.data('off')) return; //不触发事件


            var oFather = othis.parent()
                , key = othis.data('key')
                , configIndex = key.split('-')[2]//配置索引
                , index = oFather.data('index')//数据索引
                , data = table.cache[that.key][index]//数据
                , colConfig = that.config.cols[0][configIndex]
                , selfKey = index + '-' + key
                , filterRow = that.key + '-' + ELEM_EDIT + '-' + configIndex + '-' + (index)
                , filterSelect = that.key + '-' + ELEM_EDIT + '-form-' + configIndex + '-' + (index);


            //console.log(data, configIndex, colConfig);


            //显示编辑表单
            if (editType) {

                if (colConfig.type == 'switch') {
                    $('.' + classSelect).hide();
                    return;
                }
                ...
                if (colConfig.type == 'select') {
                    $('.' + ELEM_EDIT).hide();
                    $('.' + filterSelect).show().find('.layui-form-select').addClass('layui-form-selected');
                    form.on('select(' + filterRow + ')', function (o) {
                        input.val(o.value).trigger('change').trigger('blur');
                        $(o.othis).parent().hide();
                    });
                    $('.' + classSelect).each(function (i, el) {
                        let td = $(el).parent(),
                            tdKey = td.data('key'), trKey = td.parent().data('index'),
                            oKey = trKey + '-' + tdKey;
                        if (oKey !== selfKey) {
                            $(el).hide();
                        }
