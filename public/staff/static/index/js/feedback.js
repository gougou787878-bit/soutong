	...
	function ajaxFileUpload(img)
	{
		...
		$("."+img).animate({"width":"100%"},700,function(){
			$.ajaxFileUpload
			({
				url: '/index.php?m=feedback&a=upload',
				secureuri: false,
				fileElementId: id,
				data: { },
				dataType: 'html',
				success: function(data){
					...
					if(str.ret==200)
					{
                        ...
                        $("#img_file1").attr("src",picDomain +  str.data.url);
				...
				error: function(data) 
				{
                    ...
                    $(".shadd[data-select="+img+"]").hide();
                    $("img[data-index="+img+"]").attr("src","/static/index/img/no2.jpg");
				}
			  })
			return true;
		});
    ...
    function check_input() {
        ...
        if ( content != '') {
            $("#save_btn").css({"background-image": "linear-gradient(to right, #ed00ff, #ff0089)"});
    ...
    function save() {
        ...
        $.ajax( {
            type: "post",
            url: "./index.php?m=Feedback&a=feedbackSave",
            dataType: "json",
            cache:false,
            timeout : 30000,
            data: {
...
$(function(){
    $("#content").css({"height":$(window).height() *0.4+"px"});
