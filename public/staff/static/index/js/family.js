	...
	function upload(index)
	{
		...
		if(window.addEventListener){
			iptt.addEventListener('change',function(){
					...
					/*   var arr_img=new Array("/public/static/index/img/identity_face.png","/public/static/index/img/identity_back.png","/public/static/index/img/identity_handle.png");
	...
	function ajaxFileUpload(img)
	{
		...
		$("."+img).animate({"width":"100%"},700,function(){
			$.ajaxFileUpload
			({
				url: '/index.php?m=Family&a=upload',
				secureuri: false,
				fileElementId: id,
				data: { },
				dataType: 'html',
				success: function(data){
					...
					if(str.ret==200)
					{
						...
						if(id=="ipt-file1")//勋章
						{
							...
							$(".box-upload[data-index="+img+"] img").attr("src","http://imgpublic.ycomesc.com/img.live//index/img/ok2.jpg");
						}
					...
					else
					{
						...
						if(id=="ipt-file1")//勋章
						{
							$(".shadd[data-select="+img+"]").hide();
							$(".box-upload[data-index="+img+"]").show();
							$(".box-upload[data-index="+img+"] img").attr("src","/static/index/img/no2.jpg");
						}
						else if(id=="ipt-file2")
						{
							$(".shadd[data-select="+img+"]").hide();
			...
			success: function(data)
			{
				if(data.state==0){
					...
					layer.alert('你的家族'+data['name']+'申请成功',
					{
						...
						window.location.href="/index.php?m=Family&a=home&uid="+data['uid']+"&token="+data['token'];
					});
				}
			}
		});
	...
	function examine_edit(_this,touid,type,pass)
	{
		isbuy=0;
		$.ajax({
			url:'/index.php?m=Family&a=examine_edit',
			type:'POST',
	...
	function signout_post(_this,touid,type,reason)
	{
		isbuy=0;
		$.ajax({
			url:'/index.php?m=Family&a=signout_post',
			type:'POST',
...
$(function(){
	...
	$(".attended .search_btn").on("click",function(){
		...
		isbuy=0;
		$.ajax({
			url:'/index.php?m=Family&a=attended_search',
			type:'POST',
			...
			success:function(data){
				...
				if(data.code==0){
					...
					for(var i=0;i<length;i++){
						html+='<li>';
						html+='<a href="index.php?m=Family&a=detail&familyid='+list[i]['id']+'&uid='+uid+'&token='+token+'">';
	...
	$(".attended .list_title .reload").on("click",function(){
		...
		isbuy=0;
		$.ajax({
			url:'/index.php?m=Family&a=attended_reload',
			type:'POST',
			data:{uid:uid},
			dataType:'json',
			success:function(data){
	...
	$(".apply_wait .revoke").on("click",function(){
		...
		isbuy=0;
		$.ajax({
			url:'/index.php?m=Family&a=revoke',
			type:'POST',
	...
	$(".home .edit_divide").on("click",function(){
		...
		layer.prompt({
			...
			isbuy=0;
			$.ajax({
				url:'/index.php?m=Family&a=setdivide',
				type:'POST',
	...
	$(".setdes .setdes_post").on("click",function(){
		...
		isbuy=0;
		$.ajax({
			url:'/index.php?m=Family&a=setdes_post',
			type:'POST',
	...
	$(".member .user-list ul li .action span.ok").on("click",function(){
		...
		layer.prompt({
			...
			isbuy=0;
			$.ajax({
				url:'/index.php?m=Family&a=member_setdivide',
				type:'POST',
	...
	$(".member .user-list ul li .action span.no").on("click",function(){
		...
		layer.confirm('要把成员'+name+'踢出家族吗？',{
			...
			layer.close(index);
			layer.prompt({title: '踢出理由', formType: 2, btn: ['确定','']}, function(pass, index){
				if(hasEmoji(pass)){
					layer.msg("不能含有表情");
					return !1;
				}
				...
				isbuy=0;
				$.ajax({
					url:'/index.php?m=Family&a=member_del',
					type:'POST',
	...
	$(".detail_sign .sign").on("click",function(){
		...
		isbuy=0;
		$.ajax({
			url:'/index.php?m=Family&a=attended_add',
			type:'POST',
			...
			success:function(data){
				...
				if(data.code==0){
					layer.msg(data.msg,{},function(){
						window.location.href="/index.php?m=Family&a=home&uid="+uid+"&token="+token;
					});
				}else{
					layer.msg(data.msg);
					return !1;
	...
	$(".attended_wait .revoke").on("click",function(){
		...
		isbuy=0;
		$.ajax({
			url:'/index.php?m=Family&a=attended_revoke',
			type:'POST',
			data:{uid:uid,token:token},
			dataType:'json',
			success:function(data){
	...
	$(".relieve .relievebtn").on("click",function(){
		...
		isbuy=0;
		$.ajax({
			url:'/index.php?m=Family&a=retreat',
			type:'POST',
			data:{uid:uid,token:token},
			dataType:'json',
			success:function(data){
